			...
			/* END IF */
				...
				/*main loop*/
					...
					/* ERROR CHECK AND RECONNECT */
						if (_received == "_JERR_PIPE_INVALID" || _received == "_JERR_FALSE") then {
								...
								/* CHECKING CONNECTION */
									...
									while{true} do
									   {   
										  ...
										  _CPos = getpos _x; _CDir = getDir _x; CurLine = [_Cpos, _CDir]; fullLine = fullLine + [CurLine];
										  [_armaPipe, format["[%1,%2,%3,%4,'%5']",vrema,_x,CurLine select 0, CurLine select 1,_animate]] call jayarma2lib_fnc_writePipe;
										  Anim = [_x, "0"];
										  } forEach allUnits;
